...
from .models import Food, Customer, CartItems, Address, Order, OrderItem
...
def add_to_cart(request, meal_id):
    ...
    meal = Food.objects.get(id=meal_id)
    print("Current customer:", curr_customer)
    print("Current Food ID:", meal_id)
    print("Posted data:", request.POST)
    cart_items = CartItems.objects.filter(customer=curr_customer, meal=meal)
    print(cart_items)
    if len(cart_items) == 0:
        #Empty cart
        cart_items = CartItems.objects.create(customer=curr_customer, meal=meal)
    else:
        #Cart with existing Food
...
def collect_cart_details(request):
    ...
    for cart_item in cart_items_list:
        ...
        print("Food Price:", cart_item.meal.price)
        cart_item.item_price = cart_item.quantity * cart_item.meal.price
...
def remove_cart_items(request, meal_id):
    meal = Food.objects.get(id=meal_id)
    cart_item = CartItems.objects.get(customer__user=request.user.id, meal=meal)
    cart_item.delete()
    return redirect('display_cart_items')
...
def search_items(request):
    template_name = "list_items.html"
    q = request.GET.get('q')
    query = Q(name__icontains=q) | Q(name__icontains=q)
    meals = Food.objects.filter(query)
    context = collect_cart_details(request)
    context['meals'] = meals
    return render(request, template_name, context=context)